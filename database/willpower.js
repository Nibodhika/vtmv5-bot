module.exports = function(db){
    ...
    // Prepare the queries
    const do_get = db.prepare('SELECT willpower, w_superficial, w_aggravated FROM character WHERE player=?');
    const do_set = db.prepare('UPDATE character SET willpower=? WHERE player=?');
    const do_set_superficial = db.prepare('UPDATE character SET w_superficial=? WHERE player=?');
    const do_set_aggravated = db.prepare('UPDATE character SET w_aggravated=? WHERE player=?');
